...
export function LoginForm() {
  ...
  const handleSocialLogin = async (provider: "google" | "apple") => {
    try {
      ...
      console.error(`Errore durante il login con ${provider}:`, err)
      setError(`Si è verificato un errore durante il login con ${provider}. Riprova più tardi.`)
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button variant="outline" type="button" disabled={!!socialLoading} onClick={() => handleSocialLogin("google")}>
        ...
        <Button variant="outline" type="button" disabled={!!socialLoading} onClick={() => handleSocialLogin("apple")}>
          {socialLoading === "apple" ? (
            <span className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" />
          ) : (
